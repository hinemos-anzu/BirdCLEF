# BirdCLEF+ 2026 — v8: Partial Label File Utilization
#
# Change from v5.1 (0.947 baseline):
#   Partial training files (1-11 labeled windows, < full 12) are now included
#   when building the Perch cache, and their windows are used for:
#     - MLP probe training (emb_tr_aug / sc_tr_aug / Y_aug_aligned)
#     - Prior table building (already used full sc/Y_SC — no change needed)
#   SSM training (LightProtoSSM, ResidualSSM) still uses full-file data only,
#   since those models require complete 12-window sequences.
#   No BirdNET, no TTA — pure 60/40 ProtoSSM/SED blend.
#
# Attribution: Vyanktesh Dwivedi (base), Imaad Mahmood (ProtoSSM 0.946),
#              Tucker Arrants (SED)

# ── Cell 1: ProtoSSM pipeline ─────────────────────────────────────────────────
...
partial_files = sorted(windows_per_file[
    (windows_per_file > 0) & (windows_per_file < N_WINDOWS)
].index.tolist())
all_train_files = full_files + partial_files
...
all_labeled_rows = (sc.sort_values(["filename", "end_sec"])
                    .reset_index(drop=False))

print(f"Classes: {N_CLASSES} | Fully-labeled files: {len(full_files)} | Partially-labeled: {len(partial_files)}")
print(f"Full-file windows: {len(full_rows)} | All labeled windows: {len(all_labeled_rows)} | Active classes: {int((Y_FULL.sum(0) > 0).sum())}")
...
def _build_cache():
    print(f"Building Perch cache from {len(all_train_files)} files "
          f"({len(full_files)} full + {len(partial_files)} partial)…")
    train_paths = [BASE / "train_soundscapes" / fn for fn in all_train_files]
...
# Validate only full-file rows (partial files are not in full_rows)
_row_id_to_sc_idx = all_labeled_rows.set_index("row_id")["index"]
_full_row_ids = set(full_rows.set_index("row_id").index)
_full_in_cache = meta_tr[meta_tr["filename"].isin(set(full_files))]
missing_rows = set(_full_in_cache["row_id"]) - _full_row_ids
if missing_rows:
    raise RuntimeError(f"Cache has {len(missing_rows)} full-file row_ids not in labeled set.")

# Augmented data (full + partial) for MLP probes
meta_tr_all = meta_tr.copy()
sc_tr_all   = sc_tr.copy()
emb_tr_all  = emb_tr.copy()
_labeled_mask = meta_tr_all["row_id"].isin(set(_row_id_to_sc_idx.index))
sc_tr_aug   = sc_tr_all[_labeled_mask.values]
emb_tr_aug  = emb_tr_all[_labeled_mask.values]
meta_tr_aug = meta_tr_all[_labeled_mask].reset_index(drop=True)
Y_aug_aligned = Y_SC[_row_id_to_sc_idx.loc[meta_tr_aug["row_id"]].to_numpy()]

# Full-file data for SSM (requires complete 12-window sequences)
_full_mask = meta_tr["filename"].isin(set(full_files))
meta_tr = meta_tr[_full_mask].reset_index(drop=True)
sc_tr   = sc_tr[_full_mask.values]
emb_tr  = emb_tr[_full_mask.values]
Y_FULL_aligned = Y_SC[_row_id_to_sc_idx.loc[meta_tr["row_id"]].to_numpy()]
print(f"SSM data (full): sc_tr={sc_tr.shape}  emb_tr={emb_tr.shape}  Y_FULL_aligned={Y_FULL_aligned.shape}")
print(f"MLP data (aug):  sc_tr_aug={sc_tr_aug.shape}  emb_tr_aug={emb_tr_aug.shape}  Y_aug_aligned={Y_aug_aligned.shape}")
...
# ── Cell 3: BirdNET v2.4 (third branch, graceful fallback) ───────────────────
...
if _bn_model_path is None:
    USE_BIRDNET = False
    print("BirdNET model not found — Gate 3b inactive")
    BN_TO_COMP = {}
    BN_PROXY   = {}
else:
    USE_BIRDNET = True
...
# ── Cell 4: Blend + post-processing (v5: 60/40 fixed, BirdNET = Gate 3b only) ─
import os
import numpy as np
import pandas as pd
from pathlib import Path
