...
✅ V18 CFG loaded
  n_epochs=40  patience=8  oof_n_splits=3  mlp_max_iter=200
Config ready
  run_oof=False  verbose=False  dryrun=0
...
Unmapped species total:        31
Species with genus proxy:      3
Species still without signal:  28

Proxy targets:
  1491113      (Amphibia  ) ← 9 Perch genus matches
  1595929      (Amphibia  ) ← 2 Perch genus matches
  25073        (Amphibia  ) ← 6 Perch genus matches
...
✅ Perch inference engine (ONNX + multithreaded I/O) defined
...
USE_ONNX = True  (cache will be built with ONNX)
No cache found — building from scratch (~1.5 min)
Building Perch cache from 59 training files…
  Perch pass done in 105.1s  scores=(708, 234) embs=(708, 1536)
  Cache saved to /kaggle/working/cache
Loading Perch cache from: /kaggle/working/cache
  scores ← 'scores'  shape=(708, 234)
  embs   ← 'embs'  shape=(708, 1536)
  primary_labels schema OK
...
✅ File-level confidence scaling defined
...
✅ Temperatures: 171 event species (T=1.10), 63 texture species (T=0.95)
...
✅ Vectorized MLP probe inference defined
...
✅ CHANGE 2: Isotonic calibration + per-class threshold optimization defined
...
✅ Adaptive delta smoothing defined
...
✅ CHANGE 4: LightProtoSSM with cross-attention (2 heads) + SWA defined
...
✅ CHANGE 3: TTA with 5 circular shifts defined
...
✅ ResidualSSM defined (~439K params, ~20s training)
...
Submit mode: skipping OOF evaluation
...
LightProtoSSM trained — best loss=0.9862
ProtoSSM training: 9.8s
Embedding: (708, 1536) → PCA: (708, 64)  (variance retained: 81.48%)
Training MLP probes for 58 species (>= 5 pos windows)...
Trained 58 MLP probes
Calibrated 31 classes
Mean threshold: 0.467
Range: [0.25, 0.50]
Residuals: mean=-0.2120  std=0.2057  abs_mean=0.2227
ResidualSSM params: 176,010
ResidualSSM trained — best val MSE=0.024478
Correction magnitude: mean_abs=0.2117  max=0.7430
ResidualSSM training: 1.2s
Correction applied — mean_abs=0.2126  score range [-8.479, 7.315]

submission.csv saved — shape (240, 235)
Total wall time: 2.9 min
...
# Cell 3 — Logit Blend + Dual Fat-tail continuity + Prob-based Rescues
import numpy as np
import pandas as pd
from pathlib import Path
...
blended_logit = to_logit(pa) * (1.0 - SED_W) + to_logit(pb) * SED_W
pred = to_prob(blended_logit)

# ── Ranks for threshold conditions only ──
...
pred = np.where(fake_only, (1.0 - FAKE_ONLY_BLEND) * pred + FAKE_ONLY_BLEND * pa, pred)


# ── Fat-Tail Kernel Builder ──
def get_fat_tail_kernel(radius, df, scale):
    offs = np.arange(-radius, radius + 1, dtype=np.float32)
    kernel = (1.0 + (offs / scale) ** 2 / df) ** (-(df + 1.0) / 2.0)
    return (kernel / kernel.sum()).astype(np.float32)


# ── 2) Proto temporal-continuity rescue ──
PROTO_CONT_RADIUS    = 3
PROTO_CONT_DF        = 2.0
...
proto_kernel = get_fat_tail_kernel(PROTO_CONT_RADIUS, PROTO_CONT_DF, PROTO_CONT_SCALE)

# ── 3) SED temporal-continuity rescue ──
SED_CONT_RADIUS      = 2
SED_CONT_DF          = 2.0
SED_CONT_SCALE       = 1.00
...
sed_kernel = get_fat_tail_kernel(SED_CONT_RADIUS, SED_CONT_DF, SED_CONT_SCALE)
...
for fid in pd.unique(file_ids):
    ...
    x_b = pb[m]
    if len(x_a) > 1:
        xp_a = np.pad(x_a, ((PROTO_CONT_RADIUS, PROTO_CONT_RADIUS), (0, 0)), mode="edge")
        pa_ctx[m] = sum(proto_kernel[i] * xp_a[i:i + len(x_a)] for i in range(2 * PROTO_CONT_RADIUS + 1))

        xp_b = np.pad(x_b, ((SED_CONT_RADIUS, SED_CONT_RADIUS), (0, 0)), mode="edge")
        pb_ctx[m] = sum(sed_kernel[i] * xp_b[i:i + len(x_b)] for i in range(2 * SED_CONT_RADIUS + 1))

# Ranks for conditions only
...
# Proto Rescue — probability-based addition
proto_cont = (
    (xctx_a > PROTO_CONT_RANK_THR) &
...
pred = np.where(
    proto_cont,
    (1.0 - PROTO_CONT_BLEND) * pred + PROTO_CONT_BLEND * np.maximum(pa, pa_ctx),
    pred,
)

# SED Rescue — probability-based addition
sed_cont = (
...
pred = np.where(
    sed_cont,
    (1.0 - SED_CONT_BLEND) * pred + SED_CONT_BLEND * np.maximum(pb, pb_ctx),
    pred,
)

# ── 4) Rare SED local spike rescue ──
...
pred = np.where(sed_only, (1.0 - SED_ONLY_BLEND) * pred + SED_ONLY_BLEND * pb, pred)
...
print(f"Blend: Logit ({1.0 - SED_W:.2f} ProtoSSM : {SED_W:.2f} SED) + Prob-based Rescues")
