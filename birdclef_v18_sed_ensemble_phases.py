...
# ── Cell 10: SED Ensemble Phases (Phase 0 = Champion Reproduce) ────────
# Phase 0: Champion reproduction (no changes)
# Phase 1: + Mirror correction only
# Phase 2: + B0/B3 SED + Rank Average + Mirror
# Phase 3: + SED v5 ONNX 8 models + Rank Average + Mirror
# Phase 4: Rank blend weight sweep

# ──────────────────────────────────────────────────────────────────────
# EXPERIMENT CONFIGURATION — change only this section
# ──────────────────────────────────────────────────────────────────────
CURRENT_PHASE = 0   # 0,1,2,3,4
PHASE4_TRIAL  = 1   # phase 4 only: 1=(0.70,0.30) 2=(0.75,0.25) 3=(0.65,0.35) 4=(0.80,0.20)

# Champion fixed values — NEVER CHANGE
CORRECTION_WEIGHT         = 0.30
ENSEMBLE_W                = 0.5
PRIOR_LAMBDA              = 0.4
...
print(f"  changed variable     : {_CHG_VAR}")
print(f"  previous value       : {_PREV_VAL}")
print(f"  new value            : {_NEW_VAL}")
print(f"  CORRECTION_WEIGHT    : {CORRECTION_WEIGHT}")
print(f"  ENSEMBLE_W           : {ENSEMBLE_W}")
print(f"  PRIOR_LAMBDA         : {PRIOR_LAMBDA}")
print(f"  APPLY_THRESHOLD_TRANSFORM: {APPLY_THRESHOLD_TRANSFORM}")
print(f"  Threshold transform  : ENABLED")
print("=" * 70)

_cell10_start = time.time()

# Note: seeds intentionally NOT fixed.
# seed=42 consistently degraded LB by ~0.003 vs no-seed runs.
# Mirror delta (+0.001) is stable regardless of seed, so absolute
# score is more important than run-to-run reproducibility here.
...
# ── Step B: ProtoSSM inference on TEST ────────────────────────────────
...
print(f"ProtoSSM test logits: {proto_scores_flat.shape}  "
      f"range=[{proto_scores_flat.min():.3f}, {proto_scores_flat.max():.3f}]")

# ── Step C: Prior tables ───────────────────────────────────────────────
prior_tables   = build_prior_tables(sc, Y_SC)
sc_te_adjusted = apply_prior(sc_te,
    sites=meta_te["site"].to_numpy(), hours=meta_te["hour_utc"].to_numpy(),
    tables=prior_tables, lambda_prior=PRIOR_LAMBDA)
...
print(f"MLP test logits: {sc_te_adjusted.shape}  "
      f"range=[{sc_te_adjusted.min():.3f}, {sc_te_adjusted.max():.3f}]")

# ── Step E: First-pass ensemble (ENSEMBLE_W = 0.5 FIXED) ──────────────
first_pass_flat = ENSEMBLE_W * proto_scores_flat + (1.0 - ENSEMBLE_W) * sc_te_adjusted

# ── Step F: ResidualSSM — build training first-pass, calibrate, train ─
...
sc_tr_prior = apply_prior(sc_tr,
    sites=meta_tr["site"].to_numpy(), hours=meta_tr["hour_utc"].to_numpy(),
    tables=prior_tables, lambda_prior=PRIOR_LAMBDA)
...
t0 = time.time()
res_model, _cw_returned = train_residual_ssm(
    emb_full=emb_tr, first_pass_flat=first_pass_tr, Y_full=Y_FULL_aligned,
    site_ids=tr_site_ids, hour_ids=tr_hour_ids,
    n_epochs=30, patience=8, lr=1e-3,
    correction_weight=CORRECTION_WEIGHT, verbose=False)
print(f"ResidualSSM training: {time.time()-t0:.1f}s")
assert _cw_returned == CORRECTION_WEIGHT, "CORRECTION_WEIGHT was mutated"
...
with torch.no_grad():
    test_correction = res_model(
        ...
        site_ids=torch.tensor(test_site_ids, dtype=torch.long),
        hours   =torch.tensor(test_hour_ids, dtype=torch.long)).numpy()
...
if APPLY_THRESHOLD_TRANSFORM:
    probs = apply_per_class_thresholds(probs, PER_CLASS_THRESHOLDS)
probs = np.clip(probs, 0.0, 1.0)

# Champion probs complete — copy before phase post-processing
champion_probs = probs.copy()
print(f"Champion probs: shape={champion_probs.shape}  "
      f"range=[{champion_probs.min():.6f}, {champion_probs.max():.6f}]  "
      f"mean={champion_probs.mean():.6f}")
...
def _apply_mirror(p):
    ...
    for g in MIRROR_PAIRS:
        ...
        if len(idx) >= 2:
            mx = p[:, idx].max(axis=1, keepdims=True)
            for ix in idx:
                p[:, ix] = mx.squeeze()
            n_ok += 1
    print(f"  mirror: {n_ok}/{len(MIRROR_PAIRS)} groups applied")
    return p


def _load_onnx_sessions(path_list, label="SED"):
    if not _ONNX_AVAILABLE:
        raise RuntimeError("ONNX Runtime not available. Required for Phase 2/3/4.")
    sessions = []
    for p in path_list:
        if not Path(p).exists():
            raise FileNotFoundError(
                f"{label} model not found: {p}\n"
                "Add the dataset containing this model to Kaggle inputs.")
        _so = ort.SessionOptions()
        _so.intra_op_num_threads = 2
        sess = ort.InferenceSession(str(p), sess_options=_so,
                                    providers=["CPUExecutionProvider"])
        sessions.append(sess)
        print(f"  loaded {Path(p).name}  "
              f"in={sess.get_inputs()[0].shape}  "
              f"out={sess.get_outputs()[0].shape}")
    return sessions
...
def run_sed_inference(audio_paths, sessions):
    all_probs = []
    for path in tqdm(audio_paths, desc="SED"):
...
def rank_average_blend(champ_p, sed_p, pw, sw):
    from scipy.stats import rankdata
    out = np.empty_like(champ_p, dtype=np.float32)
    n   = len(champ_p)
    for ci in range(N_CLASSES):
        pr = rankdata(champ_p[:, ci]).astype(np.float32)
        sr = rankdata(sed_p[:, ci]).astype(np.float32)
        out[:, ci] = (pw * pr + sw * sr) / n
...
# Phase dispatch
# ──────────────────────────────────────────────────────────────────────
probs           = champion_probs.copy()
sed_model_count = 0
sed_probs_used  = None
mirror_applied  = False

if CURRENT_PHASE == 0:
    print("[Phase 0] Champion reproduction — no additional post-processing.")

elif CURRENT_PHASE == 1:
    print("[Phase 1] Mirror correction only.")
    probs          = _apply_mirror(probs)
    mirror_applied = True
    ...
    print(f"[Phase {CURRENT_PHASE}] SED inference ({sed_model_count} models)...")
    sed_probs_used = run_sed_inference(test_paths, sed_sessions)
    assert sed_probs_used.shape == champion_probs.shape, (
        f"SED shape {sed_probs_used.shape} != champion {champion_probs.shape}")

    print(f"[Phase {CURRENT_PHASE}] Rank-average blend PW={PW} SW={SW}...")
    probs = rank_average_blend(champion_probs, sed_probs_used, PW, SW)

    print(f"[Phase {CURRENT_PHASE}] Mirror correction...")
    probs          = _apply_mirror(probs)
    mirror_applied = True

else:
    raise ValueError(f"CURRENT_PHASE must be 0-4, got {CURRENT_PHASE}")

probs = np.clip(probs, 0.0, 1.0)

# ──────────────────────────────────────────────────────────────────────
# Step J: Build and validate submission
# ──────────────────────────────────────────────────────────────────────
...
assert len(sub) == len(meta_te)
assert len(sub) == len(test_paths) * N_WINDOWS
assert not sub.isna().any().any()
assert sub["row_id"].is_unique
assert sub[PRIMARY_LABELS].shape[1] == N_CLASSES
assert np.isfinite(sub[PRIMARY_LABELS].to_numpy()).all()
assert sub[PRIMARY_LABELS].to_numpy().min() >= 0.0
assert sub[PRIMARY_LABELS].to_numpy().max() <= 1.0

out_path = Path("/kaggle/working/submission.csv")
sub.to_csv(out_path, index=False)
sub.to_csv(AUDIT_PATH, index=False)

_runtime = time.time() - _cell10_start
...
print(f"  previous value         : {_PREV_VAL}")
print(f"  new value              : {_NEW_VAL}")
print(f"  train rows             : {len(sc_tr)}")
print(f"  test rows              : {len(sc_te)}")
print(f"  classes                : {N_CLASSES}")
print(f"  train files            : {len(sc_tr) // N_WINDOWS}")
print(f"  test files             : {len(sc_te) // N_WINDOWS}")
print(f"  Champion probs range   : [{champion_probs.min():.6f}, {champion_probs.max():.6f}]")
print(f"  Champion probs mean    : {champion_probs.mean():.6f}")
print(f"  SED model count        : {sed_model_count}")
if sed_probs_used is not None:
    print(f"  SED probs range        : [{sed_probs_used.min():.6f}, {sed_probs_used.max():.6f}]")
    print(f"  SED probs mean         : {sed_probs_used.mean():.6f}")
else:
    print(f"  SED probs range        : N/A")
    print(f"  SED probs mean         : N/A")
print(f"  Rank blend PW          : {PW if CURRENT_PHASE >= 2 else 'N/A'}")
print(f"  Rank blend SW          : {SW if CURRENT_PHASE >= 2 else 'N/A'}")
print(f"  Mirror applied         : {mirror_applied}")
print(f"  Final probs min        : {probs.min():.6f}")
print(f"  Final probs max        : {probs.max():.6f}")
print(f"  Final probs mean       : {probs.mean():.6f}")
print(f"  submission shape       : {sub.shape}")
print(f"  submission path        : {out_path}")
print(f"  audit copy path        : {AUDIT_PATH}")
print(f"  runtime seconds        : {_runtime:.1f}s")
